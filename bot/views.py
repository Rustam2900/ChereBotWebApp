...
class BotCompanyOrderCreateView(APIView):

    def post(self, request, bot_company_id):
        data = request.data
        data['bot_company_id'] = bot_company_id
        serializer = BotCompanyOrderSerializer(data=data)
        if serializer.is_valid():
            serializer.save()
            return Response({"message": "Buyurtma muvaffaqiyatli yaratildi!"}, status=status.HTTP_201_CREATED)
